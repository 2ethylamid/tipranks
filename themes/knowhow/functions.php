...
function st_theme_setup() {
	...
	*/
	...
	add_image_size( 'post', 671, 381, false ); // Post thumbnail
    add_image_size( 'know-thumbnail', 671, 381, false ); // Post thumbnail
...
function custom_meta_description($desc) {
    if (empty($desc)) {
        $desc = __('Read more on our blog.', 'framework');
    }

    return $desc;
}

add_filter('wpseo_twitter_description', 'custom_meta_description');
add_filter('wpseo_opengraph_desc', 'custom_meta_description');

if (WP_ENV === 'production') {

    add_action('wp_footer', function () {
        echo PHP_EOL . get_field('google_analytics', 1106);
        echo PHP_EOL . get_field('yandex_metrika', 1106) . PHP_EOL;
    });
...
function add_google_fonts() {
    wp_enqueue_style( 'wpb-google-fonts', 'https://fonts.googleapis.com/css?family=Roboto:100,200,300,400,600|PT+Sans|PT+Sans+Narrow', false );
}
