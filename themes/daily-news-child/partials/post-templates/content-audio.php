...
<article class="post-wrap <?php echo($wp_query->current_post == 0 ?  'col-sm-12 first-post' : 'col-sm-6'); ?> <?php echo(is_single() ? 'full-post' : ''); ?><?php echo daily_news_post_class(); ?>">
	<div class="featured-media cat-pos-rel<?php if (!has_post_thumbnail()) echo ' no-image' ?>">
		<?php
		$audio_host_type = rwmb_meta( 'daily_news_audio_host_type', $args = array('type' => 'radio'), $post->ID );
		$audio_self_hosted = rwmb_meta( 'daily_news_shaudio', $args = array('type' => 'file_advanced'), $post->ID );
		...
		<?php if (is_single() && $audio_host_type =='embeded'): ?>
				<?php echo rwmb_meta( 'daily_news_audio_embed_code', $args = array('type' => 'textarea'), $post->ID ); ?>
		<?php elseif(is_single() && $audio_host_type =='selfhosted' && $audio_self_hosted != NULL): ?>
			<div class="image-container align-center selfhosted<?php if (!has_post_thumbnail()) echo ' no-image' ?>" <?php if (has_post_thumbnail()) echo 'style="' . esc_attr($bg_image) . '"' ?>>
				<?php foreach ($audio_self_hosted as $audio): 
				if ($audio['title'] != '') : ?>
					<h4><span><?php echo esc_html($audio['title']); ?></span></h4>
				<?php endif; ?>
