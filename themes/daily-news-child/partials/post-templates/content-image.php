...
<article class="post-wrap <?php echo($wp_query->current_post == 0 ?  'col-sm-12 first-post' : 'col-sm-6'); ?> <?php echo(is_single() ? 'full-post' : ''); ?><?php echo daily_news_post_class(); ?>">
	<div class="featured-media<?php if (!has_post_thumbnail()) echo ' no-image' ?>">
		...
		?>
			<?php if (is_single()): ?>
				<div class="image-container magnific-popup-image">
					<?php 
						$attachment = get_post(get_post_thumbnail_id($post->ID));
						$image_caption = $attachment->post_excerpt;
						$thumbnail_src = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' );
					?>
					<a href="<?php echo esc_url($thumbnail_src[0]); ?>" <?php if ($image_caption !=NULL) : ?>data-caption="<?php echo esc_html($image_caption); ?>"<?php endif; ?>>
