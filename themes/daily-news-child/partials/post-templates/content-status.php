...
<article class="post-wrap <?php echo($wp_query->current_post == 0 ?  'col-sm-12 first-post' : 'col-sm-6'); ?> <?php echo(is_single() ? 'full-post' : ''); ?><?php echo daily_news_post_class(); ?>">
	<div class="featured-media cat-pos-rel<?php if (!has_post_thumbnail()) echo ' no-image' ?>">
		<?php
		$status_type = rwmb_meta( 'daily_news_status_type', $args = array('type' => 'radio'), $post->ID );
		$status_link = rwmb_meta( 'daily_news_status_link', $args = array('type' => 'text'), $post->ID );
		...
		<?php if (is_single() && $status_type =='twitter' && $status_link != NULL): ?>
			<div class="image-container align-center<?php if (!has_post_thumbnail()) echo ' no-image' ?>" <?php if (has_post_thumbnail()) echo 'style="' . esc_attr($bg_image) . '"' ?>>
				<blockquote class="twitter-tweet"><a href="<?php echo esc_url($status_link); ?>"></a></blockquote>
			</div>
		<?php elseif(is_single() && $status_type =='facebook' && $status_link != NULL): ?>
			<div class="image-container align-center<?php if (!has_post_thumbnail()) echo ' no-image' ?>" <?php if (has_post_thumbnail()) echo 'style="' . esc_attr($bg_image) . '"' ?>>
				<div class="fb-post" data-width="500" data-href="<?php echo esc_url($status_link); ?>"></div>
				<div id="fb-root"></div>
			</div>
		<?php else: ?>
